...
function gameStart() {
  session.service('ALTextToSpeech').then(function (tts) {
    //session.service("ALTracker ").unregisterTarget("Face")
    tts.say("Well!we will start the game.listen carefully to the instructions");
	//session.service('ALMemory').insertData("endApplication")
  }, function (error) {
    console.log(error);
  })
}
